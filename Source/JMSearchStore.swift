...
class JMSearchStore {
    ...
    private func encodeObject<T:Encodable>(_ object:T) {
        queue.async { [weak self] in
            do {
                ...
                NSKeyedArchiver.archiveRootObject(data, toFile: (self?.cachePath!)!)
            }catch let error {
    ...
    private func decodeObject<T:Codable>(_ complate:@escaping (T?)->()) {
        queue.async { [weak self] in
            guard let data = NSKeyedUnarchiver.unarchiveObject(withFile: (self?.cachePath!)!) as? Data else {
